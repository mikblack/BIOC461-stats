...
<center><img src="PNG/rstudio-web.png" height="400"></center>
...
## Data organization
 ...
 - Try to keep it simple:
    ...
    - No blank cells or rows (unless data is missing)
...
- Software packages differ in the calculation of “whisker length”.
...
## Normal curves
 ...
 and +2$\sigma$ (actually 1.96) between -3$\sigma$ and +3$\sigma$.

## Sampling Distributions
 - Start with population with a certain distribution (e.g., normal, but doesn’t have to be).
 - True mean $\mu$ and SD $\sigma$ (parameters describing the distribution)
...
## Sampling Distributions (continued)
 - May not be exactly $\mu$ and $\sigma$, but should be close, especially if $n$ is large.  
 ...
 - Called  sampling variability
    - How does taking different samples affect our estimates of $\mu$, and $\sigma$?
...
## Sampling Distribution: Normal Mean
 - _What would it look like?_
    - it would look like a normal curve
    - it would have same mean as the original distribution:  .  
    - it would have less variability:  the SD would be smaller.  
    - This is because some of the “noise” in the 
    individual observations is smoothed out in the $\bar{y}$’s.  
    - new distribution would have SD = $\sigma/\sqrt{n}$.

## Example: Y ~ N(30,6)
 - Take samples of size n = 9:   
 - Sample 1: 27.2, 33.3, 25.5, 29.7, 25.0, 28.3,  36.8, 30.0, 33.3
    - Observe $\bar{y}_1$ = 29.9.  
 - Sample 2: 26.5, 34.67, 34.7, 32.4, 32.3, 30.6, 31.4, 34.34, 26.3
    - Observe $\bar{y}_2$ = 31.5.  
 - Note that sample means are close to 30; typically much closer than the individual 18 observations. 
 - The sampling distribution of $\bar{y}$ has the same mean as $Y$ (30), but smaller SD = 6/$\sqrt{9}$ = 6/3 = 2.
...
## Central Limit Theorem
 - Even if Y doesn’t have a normal distribution, the sampling distribution of $\bar{y}$ will be close to normal, when $n$ is large. 
    - It also has mean $\mu$ and SD equal to $\sigma/\sqrt{n}$, i.e., it is approximately normal   N($\mu$, $\sigma/\sqrt{n}$) 
...
## Aside: heroin data
 - Plots of heroin data showed right skewness
    - CLT may require large $n$ to be valid (here n = 25)
 - Apply log transformation to reduce skewness
 - Construct CI on log scale and backtransform:
    - Untransformed: $\bar{y}$ = 147.7, 95% CI: (100.9, 194.5) 
    - Log$_{10}$ transform: $\bar{y}$ = 2.08, 95% CI: (1.96, 2.19) 
    - Backtransform: $\bar{y}$ = 119.4, 95% CI: (91.8, 155.3) 
 - CI for transformed data shorter (63.5) than that of untransformed data (93.6).

## Aside: heroin data
```{r}
t.test(heroin$Morphine)$conf.int
t.test(log10(heroin$Morphine))$conf.int
10^t.test(log10(heroin$Morphine))$conf.int
```
...
```
 ...
 - So we would only expect to observe a t statistic as large as this ($t_s$=-2.46) in 1.83% of samples when $H_0$ is true.
